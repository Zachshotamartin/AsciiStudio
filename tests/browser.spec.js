...
test('uploaded video plays, seeks, recolors while paused, and exports a decodable changing clip',async({page})=>{
 ...
 const event=page.waitForEvent('download');await page.getByRole('button',{name:'Export video clip',exact:true}).click();await expect(page.getByRole('button',{name:'Cancel export',exact:true})).toBeVisible();const download=await event;const bytes=await readFile(await download.path());expect(bytes.length).toBeGreaterThan(5000);
...
test('video cancellation and invalid files recover; replacing a source retains new controls',async({page})=>{
 await ready(page);await page.getByRole('button',{name:'Try a video',exact:true}).click();await expect(page.locator('[data-source-name]')).toHaveText('Color parade');await page.getByRole('button',{name:'Export video clip',exact:true}).click();await page.getByRole('button',{name:'Cancel export',exact:true}).click();await expect(page.locator('.ascii-status')).toContainText('canceled');
